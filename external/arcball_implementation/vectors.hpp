...
#include <cmath> // added this to original file of Mario Konrad
#include <numbers>
/*
#if !defined(M_PI)
...
double constexpr pi_divided_by_180 = std::numbers::pi / 180.0;
...
class vec2 {
 ...
 public:
  ...
  friend vec2 operator-(const vec2& w, const vec2& v) { return vec2(w) -= v; }
  friend vec2 operator*(const vec2& v, float f) { return vec2(v) *= f; }
  friend vec2 operator*(float f, const vec2& v) { return vec2(v) *= f; }
  friend float operator*(const vec2& a, const vec2& b) { return a.dot(b); }
...
class vec3 {
 ...
 public:
  ...
  inline vec3& rot(AXIS axis, float angle_deg) {
    switch (axis) {
      case X:
        return rotX(angle_deg);
      case Y:
        return rotY(angle_deg);
      case Z:
        return rotZ(angle_deg);
      default:
        break;
    }
    return *this;
  ...
  inline vec3& rotSphere(ANGLE angle, float angle_deg) {
    switch (angle) {
      case PHI:
        return rotPhi(angle_deg);
      case RHO:
        return rotRho(angle_deg);
      default:
        break;
    }
    return *this;
  ...
  inline vec3& rotPhi(float angle_deg) {
    return setSphere(getSphereR(), getSpherePhi() + angle_deg, getSphereRho());
  }
  inline vec3& rotRho(float angle_deg) {
    return setSphere(getSphereR(), getSpherePhi(), getSphereRho() + angle_deg);
  }
  inline float operator[](int idx) const { return x[idx]; }
